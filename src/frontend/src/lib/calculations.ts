...
export function calculateRepBalances(
  ...
  const startTimestamp = startDate
    ? BigInt(startDate.getTime() * 1_000_000)
    : undefined;
  const endTimestamp = endDate
    ? BigInt(endDate.getTime() * 1_000_000)
    : undefined;
  ...
  return reps.map((rep, repId) => {
    ...
    const repPayouts = payouts.filter((p) => {
      ...
      const matchesDate =
        (!startTimestamp || p.date >= startTimestamp) &&
        (!endTimestamp || p.date <= endTimestamp);
      return matchesRep && matchesDate;
    });

    const totalSales = repSales.reduce(
      (sum, sale) => sum + Number(sale.unitPrice) * Number(sale.quantity),
      0,
    );
    ...
    const totalPayouts = repPayouts.reduce(
      (sum, payout) => sum + Number(payout.amount),
      0,
    );

    const commissionRate =
      settings.repOverrides[repId] ?? settings.defaultCommission;
